...
class FM:
    def __init__(self, feature_num, vector_dim):
        # vector_dim代表公式里的k，为向量v的维度
        self.theta0 = 0.0   # 常数项
        self.theta = np.zeros(feature_num) # 线性项的参数
    ...
    def pred(self, x):
        # 线性部分
        lenear_term = self.theta0 + x @ self.theta
        # 双线性部分
        square_of_sum = np.square(x @ self.v)
        sum_of_square = np.square(x) @ np.square(self.v)
        # 最终预测
        y_pred = self._logistic(lenear_term + 0.5 * np.sum(square_of_sum - sum_of_square, axis=1))
...
print(f'测试集准确率：{test_acc[-1]:.4f}, \t测试集AUC：{test_auc[-1]:.4f}')
...
plt.subplot(1, 2, 1)
plt.plot(x_plot, train_acc, color='blue', label='Train Accuracy')
plt.plot(x_plot, test_acc, color='red', ls='--', label='Test Accuracy')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.legend()
