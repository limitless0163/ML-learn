...
# 统计训练集中每个用户和电影出现的数量，作为正则化的约束强度
user_cnt = np.bincount(train_data[:, 0], minlength=user_num)
item_cnt = np.bincount(train_data[:, 1], minlength=item_num)
print(user_cnt[:10])
print(item_cnt[:10])

# 用户和电影的编号要作为下标，必须要保存为整数
user_train = train_data[:, 0].astype(int)
user_test = test_data[:, 0].astype(int)
item_train = train_data[:, 1].astype(int)
item_test = test_data[:, 1].astype(int)
    ...
    with tqdm(range(max_training_step * batch_num)) as pbar:
        for epoch in range(max_training_step):
            ...
            for i in range(batch_num):
                ...
                model.update(P_grad / len(user_batch), Q_grad / len(item_batch), learning_rate)
