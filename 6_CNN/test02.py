...
# 定义图像处理方法
transform = transforms.Resize([256, 256])     # 将图像调整为256x256
...
# 加载内容图像
print('内容图像')
content_img = loadimg(content_img_path).to(device)
# 加载风格图像
print('风格图像')
style_img = loadimg(style_img_path).to(device)
...
vgg16 = models.vgg16(weights=True).features.to(device)     # 加载预训练的VGG16模型
...
for i in range(epochs):
    ...
    for cl in content_losses:
        content_score += cl.loss     # 收集内容损失
    for sl in style_losses:
        style_score += sl.loss       # 收集风格损失
    ...
    if i % 50 == 0 or i == epochs - 1:
        ...
        plt.imshow(input_img[0].permute(1, 2, 0).detach().cpu().numpy())
        plt.show()